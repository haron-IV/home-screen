    ...
    <div className="add-new-link-modal">
      ...
      <form className="add-new-link-modal__configuration" onSubmit={e => importDataFromBackupFile(e)}>
        <div className="file-input-wrapper">
          <input className="file-input" id="file" type="file" accept=".json" onChange={e => {setBackupFile(e.target.files[0])}}/>
        ...
        <div className="control-panel">
          ...
          <button className="add-link-button" onClick={() => {dispatch(toggleDataImporting())}}>Cancel</button>
        </div>
      </form>
    </div>
  );
